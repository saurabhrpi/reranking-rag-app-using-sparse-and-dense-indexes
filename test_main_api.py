...
def test_cdef_class_syntax():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        data = response.json()
        # Expect the answer to mention 'cdef class' in the content
...
def test_memory_allocation_returns_results():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        data = response.json()
        # Expect the answer to mention 'malloc', 'free', or 'memory' in the content
...
def test_speed_up_python_loop_with_c():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        data = response.json()
        # Expect the answer to mention 'for', 'cdef', or 'C code' in the content
...
def test_best_practices_mixing_python_c():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        data = response.json()
        # Expect the answer to mention 'data type', 'cdef', or 'Cython' in the content
...
def test_cython_vs_cpython():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        data = response.json()
        # Expect the answer to mention 'Cython', 'CPython', or 'performance' in the content
...
def test_llm_infinite_loop_protection(monkeypatch):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.post("/query", json={"query": "Repeat the word 'word' forever", "top_k": 3, "rerank_top_n": 1})
        data = response.json()
